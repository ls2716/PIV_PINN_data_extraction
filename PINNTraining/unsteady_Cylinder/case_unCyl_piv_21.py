...
def main(train=True, test=True):
    # case name
    case_name = "unCylinder_Foures_bc_inlet"
    case_name_title = r'PIV stride $0.02 \times 0.02$ Foures bc_inlet'
    ...
    def boundary_left_full(x, on_boundary):
        return on_boundary and not (np.isclose(x[0], v_ru[0])
                or (np.isclose(x[1], v_ru[1]) and x[0]>-1)
                or (np.isclose(x[1], v_ld[1]) and x[0]>-1))
    ...
    if train:
        data = dde.data.PDE(
            geom,
            pde,
            [bc_wall_u, bc_wall_v, bc_wall_fx, bc_wall_fy, bc_curl_fs, u_piv_points, v_piv_points],
            100,
            1600,
            solution=None,
            num_test=100,
        ...
        plot_train_points(data, [2, 5, 7], ["cylinder", "forcing", "piv"],

...
def main(train=True, test=True):
    # case name
    case_name = "unCylinder_Foures_formulation_with_pressure_full_lt"
    ...
    [x_p, y_p, v_p, u_p, p_p] = \
        generate_PIV_points(x_data, y_data, u_data, v_data,
                            p_data, 3, 3, [-0.55, -0.55], [0.55, 0.55], geom, True)

    pressure_coors = []
    pressure_vals = []
    for i in range(p_p.shape[0]):
        if x_p[i,0]**2+y_p[i,0]**2<0.251:
            pressure_coors.append([x_p[i,0], y_p[i,0]])
            pressure_vals.append(p_p[i,0])
    
    pressure_coors = np.array(pressure_coors)
    pressure_vals = np.array(pressure_vals)[:,None].reshape(-1,1)
    ...
    print('Verbosity level', dde.backend.tf.logging.get_verbosity())
    # exit(0)
    # NN model definition

...
def main(train=True, test=True):
    # case name
    case_name = "unCylinder_2nd_order_superresolutions_with_pressure_anchor_0.25"
    case_name_title = r'PIV superresolution second order 0.25 by 0.25'
    ...
    if test:
        ...
        old_l2_error = np.sum(velo_errors)/N_non_zero
        l2_error = np.sqrt(np.sum(np.power(U_err,2)))/N_non_zero
        print('old L2 error!: ', old_l2_error)

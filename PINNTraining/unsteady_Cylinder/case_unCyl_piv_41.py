...
def generate_PIV_points(x, y, outputs, x_stride, y_stride, v_ld, v_ru, geometry):
    ...
    for i in range(len(outputs_p)):
        outputs_p[i] = outputs_p[i][start_ind_y::y_stride, start_ind_x::x_stride]
...
def main(train=True, test=True):
    # case name
    case_name = "unCylinder_piv_slices"
    case_name_title = r'PIV slices stride $0.2 \times 0.2$ Re stresses uw, vw total information'
    ...
    [x_piv_u, y_piv_u, u_piv, uu_piv] = \
        generate_PIV_points(x_data, y_data, [u_data, uu_data],
                            5, 100, v_ld, v_ru, geom)
    piv_points_u = np.hstack((x_piv_u, y_piv_u))

    [x_piv_v, y_piv_v, v_piv, vv_piv] = \
        generate_PIV_points(x_data, y_data, [v_data, vv_data],
                            100, 5, v_ld, v_ru, geom)
    piv_points_v = np.hstack((x_piv_v, y_piv_v))
    ...
    # BC objects
    u_piv_points = dde.PointSetBC(piv_points_u, u_piv, component=0)
    uu_piv_points = dde.PointSetBC(piv_points_u, uu_piv, component=3)
    ...
    v_piv_points = dde.PointSetBC(piv_points_v, v_piv, component=1)
    vv_piv_points = dde.PointSetBC(piv_points_v, vv_piv, component=5)
    ...
    if train:
        data = dde.data.PDE(
            geom,
            pde,
            [bc_wall_u, bc_wall_v, bc_wall_uu, bc_wall_uv, bc_wall_vv, u_piv_points, uu_piv_points, v_piv_points, vv_piv_points],
            100,
            1600,
            solution=None,
            num_test=100,
        ...
        plot_train_points(data, [2,5,7,9], ["u,v wall BC", "uu,uv,vv wall BC", "u, uu PIV slices", "v, vv PIV slices"],
    ...
    if train:
        # Adam optimization
        loss_weights = [1, 1, 1, 1, 10, 10, 10, 10, 10, 10, 10, 10, 10]
        ...
        loss_update = dde.callbacks.LossUpdateCheckpoint(
            ...
            base_range=[0, 1, 2, 3],
            update_range=[4, 5, 6, 7, 8, 9, 10, 11, 12]
        )
        print('Training for 10000 epochs')
        losshistory, train_state = model.train(
            epochs=10000, callbacks=[checkpointer, loss_update], display_every=100
        )

...
def main(train=True, test=True):
    # case name
    case_name = "unCylinder_Foures_formulation"
    ...
    if train:
        ...
        plot_train_points(data, [4, 6], ["_cylinder", "_piv"],
    ...
    if test:
        ...
        def curl_f_alternative(X,V):
            ...
            return [-(u*du_xy+v*du_yy - 1/150*(du_xxy+du_yyy)) + (u*dv_xx+v*dv_xy - 1/150*(dv_xxx+dv_xyy)), 
                    -(u*du_xy+v*du_yy)  + (u*dv_xx+v*dv_xy),
                     (1/150*(du_xxy+du_yyy)) - (1/150*(dv_xxx+dv_xyy))]

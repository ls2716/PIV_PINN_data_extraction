...
def generate_PIV_points(x, y, outputs, x_stride, y_stride, v_ld, v_ru, geometry):
    ...
    for i in range(len(outputs_p)):
        outputs_p[i] = outputs_p[i].reshape(1501,601).T
...
def main(train=True, test=True):
    # case name
    case_name = "unNACA0012_2nd_order_Reynolds_superresolution"
    case_name_title = r'PIV stride $0.15 \times 0.15 Reynolds at airfoil$'
    ...
    if train:
        ...
        plot_train_points(data, [2,5,10, 11], ["u,v wall BC", "uu,uv,vv wall BC", "PIV data", "pressure"],

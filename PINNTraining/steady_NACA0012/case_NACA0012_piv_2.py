...
from equations import NS2D, func_zeros
...
def main(train=True, test=True):
    # case name
    case_name = "steady_NACA_limiting_resolution_with_pressure_anchor"
    ...
    # close_points = [item for item in airfoil_points if np.abs(item[0]-0.5)<0.02]
    # print(close_points)
    # exit(0)
    ...
    for i in range(x_data.shape[0]):
        if x_data[i,0]==0.5 and y_data[i,0]==0.054:
            p1 = p_data[i,0]
            print(p1)
    ...
    p_coors = np.array([[0.5, 0.054], [0.5,-0.054]])
    ...
    if train:
        data = dde.data.PDE(
            geom,
            pde,
            [bc_wall_u, bc_wall_v, pressure_points, u_piv_points, v_piv_points],
            100,
            1600,
            solution=None,
            num_test=100,
        ...
        plot_train_points(data, [2, 3, 5], ["airfoil", "pressure", "piv"],
    ...
    if train:
        # Adam optimization
        loss_weights = [1, 1, 1, 10, 10, 10, 10, 10]
